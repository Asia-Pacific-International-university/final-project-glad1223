...
// Keep this import if you still need SubmitQuestAnswerUseCase as a type argument for the provider,
// but ensure it DOES NOT define SubmitMiniPuzzleAnswerParams.
...
// import 'package:final_project/domain/services/quest_submision_service.dart'; // No longer needed here directly for the widget
import 'package:final_project/domain/usecases/submit_mini_puzzle_answer_usecase.dart'; // <-- THIS IS THE CRUCIAL IMPORT FOR SubmitMiniPuzzleAnswerParams
...
class _MiniPuzzleQuestWidgetState extends ConsumerState<MiniPuzzleQuestWidget> {
  ...
  Future<void> _handleSubmit(
    ...
    WidgetRef ref,
    // The type argument here is critical. It must reference the *correct* SubmitMiniPuzzleAnswerParams.
    // If submitMiniPuzzleAnswerUseCaseProvider provides the concrete type, you might simplify this.
    // However, keeping the full type helps with clarity and type checking.
    SubmitMiniPuzzleAnswerUseCase // This should be the concrete UseCase, not the abstract base for the parameter.
        submitMiniPuzzleUseCase, // The actual use case instance
    ...
    final params = SubmitMiniPuzzleAnswerParams(
      ...
      puzzleAnswer: submittedAnswer, // <--- CHANGED TO puzzleAnswer
      userId: userId,
    );

    final result = await submitMiniPuzzleUseCase(
        params); // This calls the 'call' method on the use case.
...
// final params = SubmitMiniPuzzleAnswerParams(
//       questId: widget.quest.id ?? '',
//       // Change 'answer' to 'puzzleAnswer' to match the constructor
//       puzzleAnswer: submittedAnswer, // <--- FIX IS HERE
//       userId: userId,
//     );
