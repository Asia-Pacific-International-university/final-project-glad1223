...
import 'package:flutter_riverpod/flutter_riverpod.dart'; // FIX: Import flutter_riverpod for ProviderScope.containerOf
import '../../presentation/providers/auth_provider.dart'; // Import AuthProvider
// FIX: Assuming this import provides the actual Riverpod provider instances like authProvider
...
class AppRouter {
  ...
  late final GoRouter router = GoRouter(
    ...
    routes: [
      ...
      GoRoute(
        ...
        builder: (context, state) {
          // FIX: Explicitly cast state.extra to Map<String, dynamic>
          // This ensures that resultData receives the correct type for QuestResultScreen.
          // Handle potential null or incorrect type gracefully if `extra` might not always be a Map.
          final Map<String, dynamic> resultData = (state.extra
                  is Map<String, dynamic>)
              ? state.extra as Map<String, dynamic>
              : <String,
                  dynamic>{}; // Provide an empty map or default values if type mismatch
          return QuestResultScreen(resultData: resultData);
        },
      ),
      GoRoute(
    ...
    redirect: (context, state) {
      // FIX: Use ProviderScope.containerOf(context).read to access Riverpod providers in redirect
      // This is the correct way to get a provider instance outside of a ConsumerWidget/ConsumerState.
      final container = ProviderScope.containerOf(context);
      final authProviderInstance =
          container.read(authProvider); // Access the AuthProvider instance

      final bool loggedIn = authProviderInstance.currentUser != null;
      ...
      if (loggedIn &&
          authProviderInstance.requiresFacultySelection() && // Use the instance
          !selectingFaculty) {
      ...
      if (loggedIn &&
          !authProviderInstance
              .requiresFacultySelection() && // Use the instance
          selectingFaculty) {
        return AppConstants.homeRoute;
      }
